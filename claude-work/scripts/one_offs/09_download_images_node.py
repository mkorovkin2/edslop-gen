...
from src.agents.images import download_images_node
...
async def main() -> None:
    ...
    updates = await download_images_node(state, openai_client)
    state.update(updates)
    save_state(state)

    print("Download images OK")
    print(f"downloaded: {state.get('metadata', {}).get('images_downloaded', 0)}")
    print(f"failed: {state.get('metadata', {}).get('images_failed', 0)}")
